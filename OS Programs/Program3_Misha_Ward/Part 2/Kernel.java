    ...
    public static int interrupt( int irq, int cmd, int param, Object args ) {
        ...
        switch( irq ) {
            ...
            switch( cmd ) {
                ...
                /*---------------------Part1 start-----------------------*/
                case WAIT: // WAIT
                myTcb = scheduler.getMyTcb();  // current TCB
                return waitQueue.enqueueAndSleep(myTcb.getTid());  // have the TID wait and add to SyncQueue

                case EXIT:  // EXIT
                myTcb = scheduler.getMyTcb();  // current TCB
                waitQueue.dequeueAndWakeup(myTcb.getPid(), myTcb.getTid());  // have the TID wake up and retrieve from queue
                scheduler.deleteThread();  // delete thread (exit)
                return OK;
                ...
                ; // busy wait
                ioQueue.enqueueAndSleep(COND_DISK_REQ);  // enqueue condition req

                while ( disk.testAndResetReady( ) == false )
                ; // busy wait
                ioQueue.enqueueAndSleep(COND_DISK_FIN);  // enqueue the condition fin
                return OK;
                ...
                ; // busy wait
                ioQueue.enqueueAndSleep(COND_DISK_REQ);  // enqueue disk req

                while ( disk.testAndResetReady( ) == false )
                ; // busy wait
                ioQueue.enqueueAndSleep(COND_DISK_FIN);  // enqueue disk fin
                return OK;
